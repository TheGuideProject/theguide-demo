...
const HEATGRID: number[][] = DAYS.map((_, d) => {
  ...
  return Array.from({ length: 24 }, (_, h) => {
    const j = (((d * 24 + h) * 2654435761) % 100) / 100;
    if (h < 7) return 0.02 + j * 0.04;
    if (h < 9) return 0.15 + j * 0.12;
    if (h < 11) return 0.4 + j * 0.12;
    if (h < 14) return (weekend ? 0.85 : 0.72) + j * 0.1;
    if (h < 16) return 0.55 + j * 0.12;
    if (h < 19) return (weekend ? 0.78 : 0.65) + j * 0.12;
    if (h < 22) return 0.5 + j * 0.15;
    return 0.15 + j * 0.1;
  });
});
...
const POIS = [
  { name: "Galleria degli Uffizi", loc: "Piazza della Signoria", visitors: "18.420", dwell: "2h 12min", sat: 4.7, saturation: 94, alert: "danger" },
  { name: "Cattedrale Santa Maria del Fiore", loc: "Piazza del Duomo", visitors: "24.110", dwell: "1h 08min", sat: 4.8, saturation: 98, alert: "danger" },
  { name: "Ponte Vecchio", loc: "Lungarno", visitors: "31.250", dwell: "28min", sat: 4.5, saturation: 82, alert: "warn" },
  { name: "Palazzo Pitti", loc: "Oltrarno", visitors: "9.640", dwell: "1h 52min", sat: 4.6, saturation: 61, alert: "ok" },
  { name: "Piazzale Michelangelo", loc: "San Niccolò", visitors: "12.880", dwell: "42min", sat: 4.7, saturation: 54, alert: "ok" },
  { name: "Basilica di Santa Croce", loc: "Santa Croce", visitors: "6.920", dwell: "1h 24min", sat: 4.5, saturation: 48, alert: "ok" }
] as const;

const EVENTS = [
  { month: "Mag", day: "03", name: "La Notte Bianca", venue: "Centro storico", expected: "38.000", status: "crowd-control" as const },
  { month: "Giu", day: "24", name: "Calcio Storico Fiorentino", venue: "Piazza Santa Croce", expected: "22.500", status: "planned" as const },
  { month: "Mag", day: "14", name: "Mostra \"Rinascimento Digitale\"", venue: "Galleria degli Uffizi", expected: "9.400", status: "active" as const },
  { month: "Apr", day: "28", name: "Mercato Centrale · Festa dei Sapori", venue: "San Lorenzo", expected: "14.200", status: "active" as const }
];

const ALERTS = [
  { sev: "danger" as const, ico: "⚠", t: "Ponte Vecchio: affluenza oltre soglia alle 16:30 — 412 persone/min", meta: "Zona Lungarno · raccomandato rerouting", time: "16:34" },
  { sev: "ok" as const, ico: "✓", t: "Evento Uffizi \"Rinascimento Digitale\" completato senza criticità", meta: "8.240 visitatori effettivi · 0 segnalazioni", time: "15:02" },
  { sev: "info" as const, ico: "ℹ", t: "Nuova struttura registrata: B&B Santo Spirito", meta: "CIR attivato automaticamente · 4 posti letto", time: "13:18" },
  { sev: "warn" as const, ico: "!", t: "18 strutture ricettive senza CIR attivo rilevate dal controllo notturno", meta: "Oltrarno (7) · Santa Croce (6) · San Frediano (5)", time: "08:05" }
];
            ...
            <div className="pa-card pa-map-card">
              ...
              <div className="pa-map-body">
                <svg viewBox="0 0 620 400" className="pa-map-svg" role="img" aria-label="Mappa di Firenze con densità visitatori">
                  ...
                  <path d="M -20 260 Q 120 230, 240 250 T 440 260 Q 540 265, 640 250" stroke="#A8C4BB" strokeWidth="18" fill="none" strokeLinecap="round" opacity="0.8" />
                  <path d="M -20 260 Q 120 230, 240 250 T 440 260 Q 540 265, 640 250" stroke="#92B3AB" strokeWidth="1.2" fill="none" />
                  <path d="M 60 80 Q 280 40, 520 70 Q 580 140, 570 260 Q 540 340, 420 370 Q 260 380, 120 350 Q 50 280, 60 80 Z" fill="#F6F0E3" stroke="#CABE9E" strokeWidth="1.2" strokeDasharray="4 3" />
                  {ZONES.map((z) => {
                  ...
                  {ZONES.map((z) => (
                    <g key={z.id}>
                      ...
                      <text x={z.x} y={z.y - 12} textAnchor="middle" fontSize="10" fontWeight="700" fill="#1F1B16" style={{ paintOrder: "stroke", stroke: "#F6F0E3", strokeWidth: 3, strokeLinejoin: "round" }}>
                        {z.label}
                      </text>
                    </g>
                  ))}
            ...
            <div className="pa-card">
              ...
              <div className="pa-bars">
                {ORIGINS.map((o) => {
                  ...
                  const toneCls = o.tone === "accent" ? "accent" : o.tone === "sage" ? "sage" : o.tone === "neutral" ? "neutral" : "";
                  return (
                ...
                <div className="pa-heat-cells">
                  ...
                  {HEATGRID.map((row, di) => (
                    <div key={di} className="pa-heat-grid-row">
                      {row.map((v, hi) => (
                        <div key={hi} className="pa-heat-cell" style={{ background: heatColor(v) }} title={`${DAYS[di]} ${hi}:00 · ${Math.round(v * 100)}%`} />
                      ))}
                    </div>
                  ))}
                </div>
